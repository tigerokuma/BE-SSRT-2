...
export class WatchlistService {
  ...
  async getWatchlist(userId?: string) {
    ...
    const transformedItems = await Promise.all(
      userWatchlist.map(async (item) => {
        ...
        const vulnerabilityData = await this.getVulnerabilityData(
          item.watchlist.watchlist_id,
        );
        ...
        return {
          ...
          description:
            item.enhancedDescription || item.watchlist.package.description, // Use enhanced description if available
          ...
          activity_score:
            item.activityScore !== undefined && item.activityScore !== null
              ? item.activityScore
              : null, // Activity score from ActivityData
  ...
  async getWatchlistItemDetails(userWatchlistId: string) {
    ...
    const transformedHealthHistory = healthHistory.map((health) => ({
      date: health.commit_date
        ? health.commit_date.toISOString().split('T')[0]
        : health.analysis_date.toISOString().split('T')[0],
    ...
    if (latestHealthData && latestHealthData.scorecard_metrics) {
      ...
      scorecardHealth = {
        date: latestHealthData.commit_date
          ? latestHealthData.commit_date.toISOString().split('T')[0]
          : latestHealthData.analysis_date.toISOString().split('T')[0],
    ...
    return {
      ...
      scorecard_health:
        transformedScorecardData.length > 0
          ? transformedScorecardData
          : scorecardHealth,
      // Vulnerability data
  ...
  private async getVulnerabilityData(watchlistId: string) {
    try {
      const vulnerabilityData =
        await this.vulnerabilityService.getVulnerabilities(watchlistId);
