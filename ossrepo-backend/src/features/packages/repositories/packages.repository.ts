...
export class PackagesRepository {
  ...
  async searchPackages(name: string): Promise<(Package | Partial<Package>)[]> {
    ...
    const exactMatch = uniqueDbResults.find(
      (pkg) => pkg.package_name.toLowerCase() === name.toLowerCase(),
    );

    if (exactMatch && this.isDataFresh(exactMatch.fetched_at)) {
      console.log(
        `Found fresh exact match for "${name}" + ${uniqueDbResults.length - 1} related packages`,
      );

      const hasStaleData = uniqueDbResults.some(
        (pkg) => !pkg.fetched_at || !this.isDataFresh(pkg.fetched_at),
      );
      if (hasStaleData) {
    ...
    try {
      ...
      if (externalResults.length === 0) {
        console.log(
          'No external results found - returning partial matches from DB',
        );
        return uniqueDbResults;
      }
      ...
      for (const dbPkg of uniqueDbResults) {
        const exists = combined.some(
          (ext) =>
            ext.package_name?.toLowerCase() ===
            dbPkg.package_name.toLowerCase(),
        );
        if (!exists) {
          combined.push(dbPkg);
        }
      ...
      console.log(
        `Returning ${sorted.length} results with "${name}" prioritized first`,
      );
  ...
  private async searchExternalAndInitiateCache(
    name: string,
    ...
    try {
      ...
      console.log(
        `Found ${npmResults.length} from NPM, fetching details for immediate response...`,
      );
      ...
      const fastTrackPromises = npmResults.map(async (searchResult) => {
        try {
          ...
          console.warn(
            `Failed to get details for ${searchResult.name}:`,
            error.message,
          );
  ...
  private async enrichAndCacheInBackground(
    packagesToEnrich: Partial<Package>[],
  ): Promise<void> {
    console.log(
      `Starting background enrichment for ${packagesToEnrich.length} packages.`,
    );
      ...
      try {
        ...
        if (npmData.repo_url?.includes('github.com')) {
          try {
            ...
            console.warn(
              `Background GitHub fetch failed for ${npmData.package_name}:`,
              githubError.message,
            );
          }
        }
        ...
        console.warn(
          `Failed to enrich and cache package ${npmData.package_name}: ${error.message}`,
        );
      }
    });
  ...
  private async enrichPackage(npmData: Partial<Package>): Promise<Package> {
    ...
    if (npmData.repo_url?.includes('github.com')) {
      try {
        ...
        console.warn(
          `GitHub fetch failed for ${npmData.package_name}:`,
          githubError.message,
        );
      }
    }
  ...
  private async refreshPackagesInBackground(name: string): Promise<void> {
    try {
      ...
      for (const pkg of stalePackages.slice(0, 5)) {
        // Limit to 5 for background job
        try {
          if (pkg.repo_url) {
            const match = pkg.repo_url.match(/github\.com\/([^\/]+)\/([^\/]+)/);
            if (match) {
              const [, owner, repo] = match;
              const githubData = await this.githubService.getRepositoryDetails(
                owner,
                repo,
              );
          ...
          console.warn(
            `Failed to refresh package ${pkg.package_name}:`,
            error.message,
          );
        }
      }
      ...
      console.log(
        `Background refresh completed for ${stalePackages.length} packages`,
      );
    } catch (error) {
  ...
  private combineNpmData(searchData: any, detailsData: any) {
    return {
      ...
      repo_url:
        searchData.repoUrl || detailsData?.repoUrl || searchData.npmUrl || '',
  ...
  private async updatePackageFromGitHub(
    packageId: string,
    githubData: any,
  ): Promise<void> {
  ...
  async getPackageSummary(name: string): Promise<Package | null> {
    ...
    const packageData = await this.findPackageByName(name);
